...
- **建议掌握度量化核算标准（严禁主观估分）**：
  - 依据作答客观证据按认知层级加权：$m = 0.20 \times S_{\text{L1}} + 0.25 \times S_{\text{L2}} + 0.25 \times S_{\text{L3}} + 0.30 \times S_{\text{L4}}$；
  - L1 概念通过对应 0.20~0.35；L2 模型变式通过对应 0.40~0.60；L3 排错诊断通过对应 0.65~0.80；L4 高考真题母题/踩分通过对应 0.85~1.00；
  - 暴露未解决严重误解（importance: high）扣减 0.15，掌握度封顶 0.60（触发复盘）。
