...
在沿斜面方向：
- 若物块静止，由共点力平衡可知：$f_{\text{静}} = mg \sin\theta$
- 若物块加速下滑，由牛顿第二定律可知：$mg \sin\theta - f_{\text{动}} = ma$
...
::: info 压轴拔高 · 高考压轴拔高·微元法与曲面斜坡冲量通法
- 若滑道不是平直斜面，而是任意光滑曲面 $y=f(x)$：
  - 支持力 $F_N$ 的方向随切线倾角 $\theta$ 动态变化，无法直接用 $F_N \cdot t$ 求冲量；
  - 必须采用**水平方向动量投影法**：由于水平方向无外力或只有特定外力，利用动量守恒或微元积分 $\int N_x dt = \Delta p_x$ 进行系统级化简，这是新高考压轴题规避繁琐列式的核心通法！
:::
...
<QuizCard
  ...
  :options='["物块受重力、支持力、静摩擦力三个力作用", "物块受重力、支持力、静摩擦力和下滑力四个力作用", "斜面对物块的支持力大小一定等于 mg", "斜面对物块的静摩擦力大小一定等于 μmg cosθ"]'
  answer="A"
  category="斜面滑块受力辨析"
  explanation="物块静止在斜面上，只受地球施加的重力、斜面施加的垂直斜面向上的支持力和沿斜面向上的静摩擦力三个力作用。'下滑力'是重力的分力，不是独立受力；支持力等于 mg cosθ；静摩擦力由平衡条件决定为 mg sinθ，尚未达到最大静摩擦力。"
/>

<StepScoreCard id="斜面滑块受力辨析-2" :criteria='[{"desc": "垂直斜面方向受力平衡方程 F_N = mg cos37° ", "points": 2}, {"desc": "滑动摩擦力公式 f = μ F_N 计算正确（8N，方向沿斜面向下）", "points": 2}, {"desc": "沿斜面方向列出平衡方程 F = mg sin37° + f ", "points": 2}, {"desc": "结果 F = 20 N 正确且带单位", "points": 1}]'>
  <template #question>
【高考大题分步踩分自测】质量为 m = 2kg 的物块放在倾角 θ = 37° 的斜面上，物块与斜面间的动摩擦因数 μ = 0.5。物块在沿斜面向上的拉力 F 作用下沿斜面向上做匀速运动（取 g = 10m/s²，sin37° = 0.6，cos37° = 0.8）。试求拉力 F 的大小。
  </template>
  <template #solution>
...
答：拉力 F 的大小为 20 N。
  ...
  <template #pitfall>
【高考踩分要点】
① 垂直斜面方向受力平衡方程 F_N = mg cos37° 得 2 分；
② 滑动摩擦力公式 f = μ F_N 计算正确（8N，方向沿斜面向下）得 2 分；
③ 沿斜面方向列出平衡方程 F = mg sin37° + f 得 2 分；
④ 结果 F = 20 N 正确且带单位得 1 分。
  </template>
</StepScoreCard>
